...
public class WeeklyCorrelation95 extends CustomAnalysisMethod {

	//------------------------------------------------------------------------
	//------------------------------------------------------------------------
	//------------------------------------------------------------------------
	
	public WeeklyCorrelation95() {
		super("Weekly_P&L_Correlation<0.95", TYPE_PROCESS_DATABANK);
	}
			...
			while(it.hasNext()) {
				...
				if (subStratName != focusStratName) {
					...
					*/
					...
					// Weekly correlation periods
					CorrelationPeriods correlationPeriods = precomputePeriods(focusStrat, subStrat, 20, corrType);
