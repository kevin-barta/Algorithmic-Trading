...
public class SaveHoldout extends CustomAnalysisMethod {
	...
	public SaveHoldout() {
		super("SaveHoldout", TYPE_PROCESS_DATABANK);
	}
		...
		for(int j = 0; j < taskNames.length; j++){
			...
			if(ArrayUtils.contains(pathnames, "Holdout")){
				...
				while(i < databankRG.size()){
        			saver.call("saveFile", databankRG.get(i), filePath + project + "/" + taskNames[j] + "/Holdout/" + databankRG.get(i).getName() + ".sqx", true);
					i++;
				}
				break;
			}
