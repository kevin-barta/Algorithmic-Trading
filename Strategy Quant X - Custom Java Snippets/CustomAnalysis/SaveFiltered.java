...
public class SaveFiltered extends CustomAnalysisMethod {
	...
	public SaveFiltered() {
		super("SaveFiltered", TYPE_PROCESS_DATABANK);
	}
	...
	public ArrayList<ResultsGroup> processDatabank(String project, String task, String databankName, ArrayList<ResultsGroup> databankRG) throws Exception {
		...
		for(int j = 0; j < taskNames.length; j++){
			...
			if(ArrayUtils.contains(pathnames, "Filtered")){
				continue;
			}
			else{
				...
				while(i < databankRG.size()){
        			saver.call("saveFile", databankRG.get(i), filePath + project + "/" + taskNames[j] + "/Filtered/" + databankRG.get(i).getName() + ".sqx", true);
					i++;
				}
				break;
			}
